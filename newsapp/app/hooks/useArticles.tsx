      ...
      const fetchArticles = async () => {
         ...
         try {
            if (sources.length === 1) {
               ...
               const [res1, res2] = await Promise.all([fetch(`http://${process.env.NEXT_PUBLIC_BASE_URL}/api/articles/${s1}`), fetch(`http://${process.env.NEXT_PUBLIC_BASE_URL}/api/articles/${s2}`)]);
