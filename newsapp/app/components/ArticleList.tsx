...
const ArticleList: React.FC<ArticleListProps> = ({ source }) => {
   ...
   return (
      <div className="mt-8 flex flex-col lg:flex-row gap-6">
            ...
            <div className="h-[600px] overflow-y-auto px-4 pb-4 space-y-2">
               ...
               {news1.map((article, index) => (
                  <button
                     ...
                     className={`w-full text-left px-3 py-2 rounded-md text-sm font-medium transition ${selectedTitle === article.articleName ? "bg-blue-100 text-blue-900 font-semibold" : "hover:bg-gray-100 text-gray-700"}`}
                  >
                     {article.articleName}
                  </button>
               ))}
         ...
         <div className="lg:w-2/3">{selectedTitle && <DisplayArticle source={source} title={selectedTitle} />}</div>
      </div>
   );
};
